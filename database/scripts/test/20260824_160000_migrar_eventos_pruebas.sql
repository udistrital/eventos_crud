-- Migracion del esquema eventos al modelo actualizado - PRUEBAS.
...
-- 2. Verifica que los objetos y datos legacy restantes coincidan con el inventario a migrar.
DO $$
DECLARE
    tabla TEXT;
...
-- 3. Descarta el calendario 2022-3, sin periodo PA equivalente en Parametros de pruebas.
CREATE TEMP TABLE tmp_calendario_descartado (
    calendario_id INTEGER PRIMARY KEY
) ON COMMIT DROP;

INSERT INTO tmp_calendario_descartado
SELECT id
FROM eventos.calendario
WHERE nombre = 'Calendario Académico 2022-3 Posgrado'
  AND periodo_id = 26
  AND nivel = 2;

DO $$
BEGIN
    IF (SELECT COUNT(*) FROM tmp_calendario_descartado) <> 1 THEN
        RAISE EXCEPTION 'No se encontro de forma unica el calendario 2022-3 que debe descartarse en pruebas';
    END IF;
END $$;

DELETE FROM eventos.calendario_evento_tipo_publico r
USING eventos.calendario_evento ce, eventos.tipo_evento te, tmp_calendario_descartado d
WHERE r.calendario_evento_id = ce.id
  AND ce.tipo_evento_id = te.id
  AND te.calendario_id = d.calendario_id;

DELETE FROM eventos.encargado_evento ee
USING eventos.calendario_evento ce, eventos.tipo_evento te, tmp_calendario_descartado d
WHERE ee.calendario_evento_id = ce.id
  AND ce.tipo_evento_id = te.id
  AND te.calendario_id = d.calendario_id;

DELETE FROM eventos.calendario_evento ce
USING eventos.tipo_evento te, tmp_calendario_descartado d
WHERE ce.tipo_evento_id = te.id
  AND te.calendario_id = d.calendario_id;

DELETE FROM eventos.tipo_evento te
USING tmp_calendario_descartado d
WHERE te.calendario_id = d.calendario_id;

DELETE FROM eventos.calendario c
USING tmp_calendario_descartado d
WHERE c.id = d.calendario_id;

-- 4. Define equivalencias de periodos y perfiles propias del ambiente de pruebas.
CREATE TEMP TABLE tmp_periodo_pruebas (
    periodo_origen_id INTEGER PRIMARY KEY,
    periodo_pruebas_id INTEGER NOT NULL UNIQUE
) ON COMMIT DROP;

-- IDs PA verificados contra la API de Parametros de pruebas el 2026-08-26.
INSERT INTO tmp_periodo_pruebas (periodo_origen_id, periodo_pruebas_id)
VALUES
    (12, 20),
    (13, 31),
    (29, 33),
...
-- Perfiles verificados en pruebas: SGA_MF=104, ASPIRANTE=276 y ESTUDIANTE=277 (2026-08-26).
...
BEGIN
    IF (SELECT COUNT(*) FROM tmp_perfil_publico) <> 2 THEN
        RAISE EXCEPTION 'No se cargaron los perfiles de pruebas ASPIRANTE=276 y ESTUDIANTE=277';
    END IF;
END $$;

-- 5. Construye los mapas temporales para consolidar tipos legacy en procesos y actividades.
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM tmp_tipo_evento_catalogo) <> 14 THEN
        RAISE EXCEPTION 'Se esperaban 14 tipos legacy asociados a PROC_ADMISIONES despues de descartar 2022-3';
    END IF;
END $$;
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM tmp_proceso) <> 10
       OR (SELECT COUNT(*) FROM tmp_tipo_evento_proceso) <> 14
       OR (SELECT COUNT(*) FROM tmp_evento_catalogo WHERE evento_catalogo_id IS NOT NULL) <> 24 THEN
        RAISE EXCEPTION 'Los mapas de procesos o eventos no tienen los conteos esperados';
    END IF;
END $$;

-- 6. Transforma los calendarios y aplica los identificadores externos de pruebas.
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM eventos.calendario) <> 12
       OR EXISTS (
           SELECT 1
           FROM eventos.calendario c
           LEFT JOIN tmp_periodo_pruebas p ON p.periodo_origen_id = c.periodo_id
           WHERE p.periodo_origen_id IS NULL
       ) THEN
        RAISE EXCEPTION 'No todos los calendarios conservados tienen equivalencia PA en Parametros de pruebas';
    END IF;
END $$;

UPDATE eventos.calendario c
SET periodo_id = p.periodo_pruebas_id
FROM tmp_periodo_pruebas p
WHERE c.periodo_id = p.periodo_origen_id;
...
-- Documento activo Creacion_Calendario verificado en pruebas el 2026-08-26.
UPDATE eventos.calendario
SET documento_id = 168374;
...
-- 7. Normaliza las fechas particulares al formato visible usado por el MID, sin zona horaria.
...
-- 8. Completa restricciones y parametria de sesiones que permanecen en el modelo vigente.
...
-- 9. Crea las tablas estructurales de catalogos, procesos y roles funcionales de gestion.
...
-- 10. Migra los datos legacy hacia los nuevos catalogos y procesos consolidados.
...
-- 11. Convierte los eventos legacy en actividades catalogadas y migra su publico dirigido.
DELETE FROM eventos.calendario_evento_tipo_publico r
...
-- 12. Crea el modelo de extensiones por programa y la auditoria append-only de actividades.
...
-- 13. Completa indices auxiliares y sincroniza las secuencias con los identificadores migrados.
...
-- 14. Documenta el esquema, las tablas y las columnas incorporadas por la migracion.
...
-- 15. Verifica estructura, comentarios, conteos y referencias ANTES DE REALIZAR COMMIT.
DO $$
DECLARE
...
BEGIN
    ...
    IF (SELECT COUNT(*) FROM eventos.calendario) <> 12
       ...
       OR (SELECT COUNT(*) FROM eventos.proceso) <> 10
       OR (SELECT COUNT(*) FROM eventos.evento_catalogo) <> 4
       OR (SELECT COUNT(*) FROM eventos.evento_catalogo_proceso_catalogo) <> 4
       OR (SELECT COUNT(*) FROM eventos.calendario_evento) <> 24
       OR (SELECT COUNT(*) FROM eventos.calendario_evento_tipo_publico) <> 22
    ...
    IF EXISTS (SELECT 1 FROM eventos.calendario WHERE documento_id <> 168374) THEN
        RAISE EXCEPTION 'Todos los calendarios de pruebas deben usar documento_id 168374';
    END IF;

    IF (SELECT ARRAY_AGG(DISTINCT periodo_id ORDER BY periodo_id) FROM eventos.calendario)
       IS DISTINCT FROM ARRAY[20, 31, 33, 36, 40, 57, 59, 61, 75, 77]::INTEGER[] THEN
        RAISE EXCEPTION 'Los periodos de los calendarios no coinciden con los IDs PA verificados en pruebas';
    END IF;

    IF EXISTS (SELECT 1 FROM eventos.calendario_evento WHERE proceso_id IS NULL OR evento_catalogo_id IS NULL)
       OR EXISTS (SELECT 1 FROM eventos.calendario_evento_tipo_publico WHERE perfil_id NOT IN (276, 277)) THEN
    ...
    IF (SELECT COUNT(*) FROM eventos.tipo_evento) <> 14
...
-- 16. Elimina las tablas legacy cuya informacion ya fue migrada o descartada.
...
-- 17. Confirma que solo permanezcan las 18 tablas del nuevo modelo.
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM information_schema.tables
